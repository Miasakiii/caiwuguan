 ...
 */
@Singleton
class AlipayCsvImporter @Inject constructor(
    ...
    companion object {
        // 支付宝 CSV 格式示例：
        // 交易时间,交易分类,交易对方,商品说明,收/支,金额,收/付款方式,交易状态,交易单号,商家订单号,备注
    ...
    private fun parseLine(line: String): Bill? {
        ...
        val direction = fields[4].trim() // 收/支/其他
        val amountStr = fields[5].trim()
        val status = fields[7].trim()

        // 跳过未完成的交易
        if (status != "交易成功" && status != "已收入" && status != "已支付") {
            return null
        }

        // 跳过特殊交易类型
        if (category in listOf("退款", "理财", "转账")) {
            return null
        }
        ...
        val (classifiedCategory, _) = categoryClassifier.classify(merchant)
